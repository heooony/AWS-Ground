...
예를 들자면 이런겁니다. 
```
집에 들어가기 위해서 우리는 열쇠를 사용해야 합니다(시간이 지나 우리는 도어락을 사용하지만요.). 그러나 이전에 사용하던 주인이 여기의 문은 내가 가지고 있는 이 키로 열어야 한다고 줍니다. 그러나 저는 우리 집만의 키는 남이 사용했던 키 말고, 내가 직접 제작해서 더 좋은 키로 바꾸고 싶거든요!
```
